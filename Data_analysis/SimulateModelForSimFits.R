...
job_array_filename <- args[2] # job_array_filename <- "parameter_means_fits_to_simulation.tsv"
...
simulate_model_for_plot2 <- function(fit_params, params_fixed){
  ...
  params_fixed$sigma_f <- fit_params["sigmaf"]
  params_fixed$prop_f <- fit_params["propf"]
  params_fixed$m <- fit_params["m"]
  params_fixed$v <- fit_params["v"]
  ...
  WFmodel_ppxSero <- WF_PPxSero$new(pars = params_fixed,
                                      ...
                                      seed = 1L)
  for (i in 1:20) {
    cluster_samples_ParamVar[i,] <- (WFmodel_ppxSero$run(i*12)[(2:(params_fixed$species_no+1)),])
  }
...
params_4_model_data <- simulate_model_for_plot2(params_fit, params_4_woFit)
...
saveRDS(params_4_model_data_rel, paste(row_index, "_", "params_4_model_data_rel.rds", sep = ""))
