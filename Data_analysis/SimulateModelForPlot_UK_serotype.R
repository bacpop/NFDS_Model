...
simulate_model_for_plot2 <- function(mcmc_run, params_loc){
  ...
  for (i in 1:length(rand_ind)) {
    ...
    WFmodel_ppxSero <- WF_PPxSero$new(pars = params_loc,
                                      ...
                                      n_threads = 4L,
                                      seed = 1L)
    cluster_samples_ParamVar[i,] <- rowSums(matrix(WFmodel_ppxSero$run(6*12)[-(1:(params_loc$species_no+1)),], nrow = params_loc$sero_no, ncol = params_loc$species_no, byrow = TRUE)) 
  }
...
if(model_version == "4"){
  # 4-param model
  fit_4param_mcmc2_probs <- readRDS(fit_file) # something like "UK_PPxSero_ggCaller_PopPUNK_4param_det_pmcmc_run2.rds"
  # fit_4param_mcmc2_probs <- readRDS("/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_08_04/UK_newPrior/UK_PPxSero_ggCaller_PopPUNK_4param_det_pmcmc_run2.rds")
  fit_4param_mcmc2_probs_pars <- mcstate::pmcmc_thin(fit_4param_mcmc2_probs, burnin = 5000, thin = 1)$pars
  
  params_4_woFit <- list(species_no = species_no, Pop_ini = as.matrix(Pop_ini), Pop_eq = (Pop_eq), Genotypes = intermed_gene_presence_absence_consensus_matrix, capacity = capacity, delta = delta, vaccTypes1 = vaccTypes1, vaccTypes2 = vaccTypes2, gene_no = gene_no, vacc_time1 = vacc_time1, vacc_time2 = vacc_time2, dt = dt, migVec = as.matrix(migVec), sero_no = sero_no, sigma_f = NA, prop_f = NA, m = NA, v = NA)
  ...
  # saveRDS(params_4_model_data_rel, "/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_10_23/UK_SimulateModelForPlots/params_4_model_data_rel.rds")
  # saveRDS(params_4_model_data_rel, "/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_11_17/UK_SimForPlot_serotypes/params_4_model_data_rel.rds")
} else if(model_version == "2"){
  # Null model
  fit_2param_mcmc2_probs <- readRDS(fit_file) # something like "UK_PPxSero_ggCaller_PopPUNK_Null_det_pmcmc_run2.rds"
  # fit_2param_mcmc2_probs <- readRDS("/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_08_04/UK_newPrior/UK_PPxSero_ggCaller_PopPUNK_Null_det_pmcmc_run2.rds")
  fit_2param_mcmc2_probs_pars <- mcstate::pmcmc_thin(fit_2param_mcmc2_probs, burnin = 5000, thin = 1)$pars
  
  params_null_woFit = list(species_no = species_no, Pop_ini = as.matrix(Pop_ini), Pop_eq = (Pop_eq), Genotypes = intermed_gene_presence_absence_consensus_matrix, capacity = capacity, delta = delta, vaccTypes1 = vaccTypes1, vaccTypes2 = vaccTypes2, gene_no = gene_no, vacc_time1 = vacc_time1, vacc_time2 = vacc_time2, dt = dt, migVec = as.matrix(migVec), sero_no = sero_no, sigma_f = -1000, prop_f = 1, m = NA, v = NA)
  ...
  # saveRDS(params_null_model_data_rel, "/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_10_23/UK_SimulateModelForPlots/params_2_model_data_rel.rds")
  # saveRDS(params_null_model_data_rel, "/Users/llorenz/Documents/PhD_Project/Code/1st_project/WF_plots_postTAC2/2025_11_17/UK_SimForPlot_serotypes/params_null_model_data_rel.rds")
}
