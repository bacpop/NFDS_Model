...
simulate_model_for_plot2 <- function(mcmc_run, params_loc){
  ...
  for (i in 1:length(rand_ind)) {
    ...
    WFmodel_ppxSero <- WF_PPxSero$new(pars = params_loc,
                                      ...
                                      n_threads = 4L,
                                      seed = 1L)
    cluster_samples_ParamVar[i,] <- (WFmodel_ppxSero$run(6*12)[(2:(params_loc$species_no+1)),]) # time point 0 is 2001, simulate until 2007
  }
...
simulate_model_for_plot2_null <- function(mcmc_run, params_loc){
  ...
  for (i in 1:length(rand_ind)) {
    ...
    WFmodel_ppxSero <- WF_PPxSero$new(pars = params_loc,
                                      ...
                                      n_threads = 4L,
                                      seed = 1L)
    cluster_samples_ParamVar[i,] <- (WFmodel_ppxSero$run(6*12)[(2:(params_loc$species_no+1)),])
  }
  cluster_samples_ParamVar
}
