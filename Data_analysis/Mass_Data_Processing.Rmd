...
```{r}
# sequencing over time Mass
Mass_time_points <- c(2001, 2004, 2007)
seq_no_per_year <- rep(0, length(2001:2007))
names(seq_no_per_year) <- 2001:2007
for (i in 1:length(Mass_time_points)) {
   seq_no_per_year[as.character(Mass_time_points[i])] <- length(which(Mass_Samples_accCodes$`Year of Isolation`==Mass_time_points[i]))
}
barplot(seq_no_per_year, xlab = "Years", ylim = c(0, 250), ylab = "Sequence Counts")
```
...
saveRDS(gene_presence_absence_2001, "COG_Mass_gene_presence_absence_2001.rds")
```
