...
Mass_UK_50_ground_truth_panaroo_dict <- rep(NA, 466500)
for (i in 1:nrow(Mass_UK_50_ground_truth_panaroo)) {
  Mass_UK_50_ground_truth_panaroo_dict[(100*(i-1))+(1:100)] <- Mass_UK_50_ground_truth_panaroo[i,1]
  names(Mass_UK_50_ground_truth_panaroo_dict)[(100*(i-1))+(1:100)] <- Mass_UK_50_ground_truth_panaroo[i,-c(1,2,3)]
}
...
Mass_UK_50_ground_truth_panaroo_bin <- Mass_UK_50_ground_truth_panaroo[,c(-2,-3)]
Mass_UK_50_ground_truth_panaroo_bin[-1,-1] <- apply(Mass_UK_50_ground_truth_panaroo_bin[-1,-1],c(1,2), convert_to_bool)
Mass_UK_50_ground_truth_panaroo_bin_int <- sapply(Mass_UK_50_ground_truth_panaroo_bin[-1,-1],as.integer)
...
Mass_UK_50_ground_truth_panaroo_listdict <- list()
for (i in 2:nrow(Mass_UK_50_ground_truth_panaroo_bin)) {
  Mass_UK_50_ground_truth_panaroo_listdict[[Mass_UK_50_ground_truth_panaroo_bin[i,1]]] <- as.integer(Mass_UK_50_ground_truth_panaroo_bin[i,-1])
}
...
for (i in 1:nrow(Mass_UK_mmseq_clusters_50_panaroo)) {
  ...
  cluster <- c(Mass_UK_50_ground_truth_panaroo_dict[Mass_UK_mmseq_clusters_50_panaroo$V1[i]], Mass_UK_50_ground_truth_panaroo_dict[Mass_UK_mmseq_clusters_50_panaroo$V3[i]])
...
mmseqs_dict_df <- as.data.frame(names(mmseqs_dict))
for (i in 1:length(names(mmseqs_dict))) {
  mmseqs_dict_df[i,2] <- paste(unique(unname(mmseqs_dict[[names(mmseqs_dict)[i]]])), collapse = ", ")
}
...
mmseqs_dict_freqs_Mass_UK <-  vector("list", length = length(unique(Mass_UK_mmseq_clusters_50_panaroo$V1)))
names(mmseqs_dict_freqs_Mass_UK) <- unique(Mass_UK_mmseq_clusters_50_panaroo$V1)
groundtruth_freqs_Mass_UK <-  vector("list", length = length(unique(Mass_UK_mmseq_clusters_50_panaroo$V1)))
names(groundtruth_freqs_Mass_UK) <- unique(Mass_UK_mmseq_clusters_50_panaroo$V1)
...
for (i in 1:nrow(Mass_UK_mmseq_clusters_50_panaroo)) {
  ...
  Mass_sum <- rep(0,50)
  UK_sum <- rep(0,50)
  ground_truth_sum <- rep(0, 100)
  for (gene in names(clusters)) {
    #print(gene)
    ground_truth_sum <- ground_truth_sum | Mass_UK_50_ground_truth_panaroo_listdict[[clusters[gene]]]
    if(Mass_or_UK_gene_dict_50[gene]=="Mass"){
      #print("Mass")
      Mass_sum <- Mass_sum | Mass_ggCaller_gene_presence_absence_listdict_50_panaroo[[Mass_gene_clust_dict_50[gene]]]
      #print(clusters[gene])
    }
    else if(Mass_or_UK_gene_dict_50[gene]=="UK"){
    UK_sum <- UK_sum | UK_ggCaller_gene_presence_absence_listdict_50_panaroo[[UK_gene_clust_dict_50[gene]]]
    }
  }
  mmseqs_dict_freqs_Mass[[clust_name]] <- sum(Mass_sum)/50
  mmseqs_dict_freqs_UK[[clust_name]] <- sum(UK_sum)/50
  mmseqs_dict_freqs_Mass_UK[[clust_name]] <- (sum(Mass_sum)/50 + sum(UK_sum)/50)/2
  #mmseqs_dict_freqs_Mass_UK[[clust_name]] <- if(sum(Mass_sum)>0 & sum(UK_sum)>0){(sum(Mass_sum)/50 + sum(UK_sum)/50)/2} else if(sum(Mass_sum)>0){sum(Mass_sum)/50} else {sum(UK_sum)/50}
    
  groundtruth_freqs_Mass_UK[[clust_name]] <- sum(ground_truth_sum)/100
}
...
plot(mmseqs_dict_freqs_Mass, groundtruth_freqs_Mass_UK, pch = 19, col = "#00000030")
plot(mmseqs_dict_freqs_UK, groundtruth_freqs_Mass_UK, pch = 19, col = "#00000030")

# Okay, all the genes along the bottom and the left are probably mismatches
# try to optimise the clustering in a way that is 1-on-1
# but isn't that best reciprocal hit...?
