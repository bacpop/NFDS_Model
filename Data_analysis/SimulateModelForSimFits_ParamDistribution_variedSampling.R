...
sample_freq <- job_array_df[row_index,3] # name of sampling strat
...
if(rep_no == 1){
  filepath <- "/nfs/research/jlees/leonie/WF_fitting_2026/AnalysisForRevision/FitModelToVaryingSampleSize_run1/"
} else if(rep_no == 2){
  filepath <- "/nfs/research/jlees/leonie/WF_fitting_2026/AnalysisForRevision/FitModelToVaryingSampleSize_run2/"
} else if(rep_no == 3){
  filepath <- "/nfs/research/jlees/leonie/WF_fitting_2026/AnalysisForRevision/FitModelToVaryingSampleSize_run3/"
} else{
  print("replicate number invalid")
}
file_name <- paste("US_based_simulation_with_freq_1_", as.character(sample_freq), "_", as.character(budget), "_4param_det_pmcmc_run2_simulation.rds", sep = "")
