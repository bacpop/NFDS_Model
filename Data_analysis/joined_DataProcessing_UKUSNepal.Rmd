...
Nepal_meta_data_gps2_selection <- Nepal_meta_data_gps2[,c(1,2,4:11, 16,21,22)]
...
Nepal_meta_data_gps_joined <- Nepal_meta_data_gps_joined[which(Nepal_meta_data_gps_joined$Clinical_manifestation == "CARRIAGE"),]
# 2001 samples
...
# find out which Nepal genomes are from carriage (now redundant)
Nepal_carriage <- c(Nepal_meta_data_gps1$Lane_id[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], Nepal_meta_data_gps2$Lane_id[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))])
#write(Nepal_carriage, file = "../../Nepal_carriage.txt")
#write(Nepal_meta_data_gps1$Lane_id[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], file = "../../Nepal_carriage_gps1.txt")
#write(Nepal_meta_data_gps2$Lane_id[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))], file = "../../Nepal_carriage_gps2.txt")
```

```{r}
# carriage month-year plot
Nepal_carriage_year_dict <- c(Nepal_meta_data_gps1$Year[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], Nepal_meta_data_gps2$Year[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))])
names(Nepal_carriage_year_dict) <- c(Nepal_meta_data_gps1$Lane_id[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], Nepal_meta_data_gps2$Lane_id[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))])

month_to_no <- 1:12
names(month_to_no) <- c("JAN","FEB","MAR","APR", "MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC")

Nepal_carriage_month_dict <- month_to_no[c(Nepal_meta_data_gps1$Month[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], Nepal_meta_data_gps2$Month[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))])]
names(Nepal_carriage_month_dict) <- c(Nepal_meta_data_gps1$Lane_id[which(Nepal_meta_data_gps1$Clinical_manifestation == "CARRIAGE")], Nepal_meta_data_gps2$Lane_id[intersect(which(Nepal_meta_data_gps2$Study_name == "COMMUNITY CARRIAGE"), which(Nepal_meta_data_gps2$Lane_id != "No genome data available"))])

Nepal_years <- 1:6
names(Nepal_years) <- unique(Nepal_meta_data_gps_joined$Year)
Nepal_numbers_year_month <- data.frame(cbind(sort(rep(unique(Nepal_meta_data_gps_joined$Year),12)), rep(1:12,length(unique(Nepal_meta_data_gps_joined$Year))), rep(0,length(unique(Nepal_meta_data_gps_joined$Year)*12))))
colnames(Nepal_numbers_year_month) <- c("Year","Month","Count")
for (i in names(Nepal_carriage_year_dict)) {
  Nepal_numbers_year_month[12 * (Nepal_years[as.character(Nepal_carriage_year_dict[i])]-1) + Nepal_carriage_month_dict[i] ,3] <- Nepal_numbers_year_month[12 * (Nepal_years[as.character(Nepal_carriage_year_dict[i])]-1) + Nepal_carriage_month_dict[i],3] + 1
}

par(mfrow=c(1,1))
plot(Nepal_numbers_year_month[,1] + 1/12 * (Nepal_numbers_year_month[,2]-1), Nepal_numbers_year_month[,3])

# based on this, yearly assignment makes a lot more sense! because it looks like sampling was done like that
# so I will revert everything to year assignment


# make yearly plots for carriage only
Nepal_numbers_year <- rep(0, length(2005:2021))
names(Nepal_numbers_year) <- as.character(2005:2021)
for (i in names(Nepal_carriage_year_dict)) {
  Nepal_numbers_year[as.character(Nepal_carriage_year_dict[i])] <- Nepal_numbers_year[as.character(Nepal_carriage_year_dict[i])] +1
}
barplot(Nepal_numbers_year, xlab = "Sequencing Years", ylim = c(0, 1200), ylab = "Sequence Counts")

barplot(Nepal_numbers_year[-c(1,2,3,4,16,17)], xlab = "Sequencing Years", ylim = c(0, 800), ylab = "Sequence Counts")
# maybe better winter? no!
```
...
#[1] 69
# 67 (after filtering for carriage only)
```
...
# now: 173
# now: 169 (GPSCs)
# now: 165 (filtered for carriage only)
...
# I will use 2009 as start population I think # no, I should use a combination of 2009, 2012 and 2014 (see next code bit)
