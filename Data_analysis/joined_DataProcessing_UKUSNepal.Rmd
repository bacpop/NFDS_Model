...
# save for paper
Nepal_meta_data_gps2_accNo <- Nepal_meta_data_gps2_v2[filter_for_carriage,c(4, 11, 19)]
Nepal_meta_data_gps2_accNo <- Nepal_meta_data_gps2_accNo[,-2]
write.csv(Nepal_meta_data_gps2_accNo,"/Users/llorenz/Documents/PhD_Project/Papers/NFDS_Model/Data/Kathmandu_AccessionNumbers.csv")

vim# check how much these two serotyping methods really differ
same <- 0
diff <- 0
diff_names <- c()
...
Nepal_meta_dataPP$GPSC <- Nepal_seq_clusters_dict[Nepal_meta_dataPP$Lane_id]
colnames(Nepal_meta_dataPP) <- c("Lane_id", "Year", "Serotype", "GPSC")
...
US_PPclustToIndex_dict <- 1:length(unique(PopPUNK_clusters$Cluster))
names(US_PPclustToIndex_dict) <- (unique(PopPUNK_clusters$Cluster))

for (i in 1:nrow(PopPUNK_clusters)) {
  PPxSero_matrix[US_PPclustToIndex_dict[as.character(PopPUNK_clusters$Cluster[i])],SerotypeToIndex_dict[PopPUNK_clusters$Serotype[i]]] <- 1
}
...
PPsero_startpop6_large <- matrix(0, nrow = length(unique(PopPUNK_clusters$Cluster)), ncol = length(unique(PopPUNK_clusters$Serotype)))
PP_expand_factor <- 15000 / sum(PP_mass_cluster_freq_1_sero)
PP_expand_factor_US_large <- 200000 / sum(PP_mass_cluster_freq_1_sero)
exp_noise <- 1000
for (i in 1:nrow(PPsero_startpop6)) {
    ...
    PPsero_startpop6_large[i,] <- (sapply((PP_mass_cluster_freq_1_sero[i,] + rexp(n = length(PP_mass_cluster_freq_1_sero[i,]), rate = exp_noise)) * PP_expand_factor_US_large, rpois, n=1)) + ceiling(PPxSero_matrix_prob[i,])
...
saveRDS(PPsero_startpop6_large, "PPsero_startpop6_large.rds")
...
which(PPsero_startpop6[(US_PPclustToIndex_dict[4]),]>0)
# 7 11 27 28 29 32
# serotypes: 15C, 23B, 4, 1, 38, 34
which(PPsero_startpop6[(US_PPclustToIndex_dict[7]),]>0)
# 8 10 24 27
# serotypes: 11A, 8, 35F, 4
which(PPsero_startpop6[(US_PPclustToIndex_dict[9]),]>0)
# 2 11 13 14 16 24 30
...
for (i in unique(Nepal_PP$GPSC)) {
  Nepal_ggCaller_intermed_consensus[-1,(Nepal_PPclustToIndex_dict[i])+1] <- apply(as.matrix(Nepal_ggCaller_intermed[-1,c(FALSE,Nepal_seq_clusters_dict[unlist(Nepal_ggCaller_intermed[1,-1])]==i)]), 1, cons_genomes)
...
for (i in 1:length(Nepal_time_points_selected)) {
  ...
  for (j in unique(Nepal_PP$GPSC)) {
    Nepal_ggCaller_year[-1,(Nepal_PPclustToIndex_dict[j])+1] <- apply(as.matrix(Nepal_ggCaller_byYear_intermed[[i]][-1,c(FALSE,Nepal_seq_clusters_dict[unlist(Nepal_ggCaller_byYear_intermed[[i]][1,-1])]==j)]), 1, cons_genomes)
  }
...
for(j in 1:length(Nepal_time_points_selected)){
  ...
  for (i in unique(Nepal_PP$GPSC)){
    ...
    Nepal_cluster_freqs[[j]][Nepal_PPclustToIndex_dict[i]] <- length(which(Nepal_meta_dataPP[Nepal_meta_IDtoIndPP[Nepal_PP[Nepal_PP$GPSC == i,"sample"]],]$Year==Nepal_time_points_selected[j]))
  }
}
...
for(j in 1:length(Nepal_time_points_selected)){
  ...
  for (i in unique(Nepal_PP$GPSC)){
    Nepal_cluster_freqs_VT[[j]][Nepal_PPclustToIndex_dict[i]] <- length(which(Nepal_meta_dataPP[Nepal_meta_IDtoIndPP[Nepal_PP[Nepal_PP$GPSC == i & Nepal_PP$VT == "VT","sample"]],]$Year==Nepal_time_points_selected[j]))
    Nepal_cluster_freqs_NVT[[j]][Nepal_PPclustToIndex_dict[i]] <- length(which(Nepal_meta_dataPP[Nepal_meta_IDtoIndPP[Nepal_PP[Nepal_PP$GPSC == i & Nepal_PP$VT == "NVT","sample"]],]$Year==Nepal_time_points_selected[j]))
  }
}

Nepal_sero_freqs <- vector(mode = "list", length = length(Nepal_time_points_selected))
for(j in 1:length(Nepal_time_points_selected)){
  Nepal_sero_freqs[[j]] <- rep(0, length(unique(Nepal_PP$Serotype)))
  for (i in unique(Nepal_PP$Serotype)){
    Nepal_sero_freqs[[j]][SerotypeToIndex_dict[i]] <- length(which(Nepal_meta_dataPP[Nepal_meta_IDtoIndPP[Nepal_PP[Nepal_PP$Serotype == i,"sample"]],]$Year==Nepal_time_points_selected[j]))
  }
}
...
for (i in 1:length(Nepal_time_points_selected)) {
  ...
  file_name_sero <- paste("Nepal_sero_freqs_", as.character(i),".rds", sep = "")
  #print(file_name_VT)
  ...
  saveRDS(Nepal_sero_freqs[[i]], file_name_sero)
}
...
colnames(Nepal_pre_vac_df) <- names(Nepal_PPclustToIndex_dict)[which(Nepal_cluster_freqs[[1]] + Nepal_cluster_freqs[[4]] + Nepal_cluster_freqs[[6]] > 1)]
...
colnames(Nepal_pre_and_post_vac_df) <- names(Nepal_PPclustToIndex_dict)[which(Nepal_cluster_freqs[[1]] + Nepal_cluster_freqs[[4]] + Nepal_cluster_freqs[[6]] > 1)]
...
expand_factor_l <- 83000 / sum(Nepal_cluster_freqs_preVac)
exp_noise <- 10
Nepal_model_start_pop_l <- (sapply((Nepal_cluster_freqs_preVac + rexp(n = length(Nepal_cluster_freqs_preVac), rate = exp_noise)) * expand_factor_l, rpois, n=1))
...
saveRDS(Nepal_model_start_pop_l, "Nepal_model_start_pop_large.rds")
```
...
Nepal_model_start_pop_l <- readRDS("Nepal_model_start_pop_large.rds")
...
#save sequence cluster frequencies
saveRDS(Nepal_mass_cluster_freq_1_sero, file = "Nepal_mass_cluster_freq_1_sero.rds")


PPsero_startpop2 <-  matrix(0, nrow = length(unique(Nepal_clusters$GPSC)), ncol = length(unique(Nepal_clusters$Serotype)))
PPsero_startpop2_large <-  matrix(0, nrow = length(unique(Nepal_clusters$GPSC)), ncol = length(unique(Nepal_clusters$Serotype)))
...
for (i in (unique(Nepal_clusters$GPSC))) {
  ...
  PPsero_startpop2_large[Nepal_PPclustToIndex_dict[i],] <- (sapply((Nepal_mass_cluster_freq_1_sero[Nepal_PPclustToIndex_dict[i],] + rexp(n = length(Nepal_mass_cluster_freq_1_sero[Nepal_PPclustToIndex_dict[i],]), rate = exp_noise)) * expand_factor_l, rpois, n=1)) + ceiling(PPxSero_matrix_prob2[Nepal_PPclustToIndex_dict[i],])
}
saveRDS(PPsero_startpop2, "Nepal_PPsero_startpop.rds")
saveRDS(PPsero_startpop2_large, "Nepal_PPsero_startpop_large.rds")
...
# do same per serotype
UK_sero_freq_1 <- rep(0,length(unique(UK_PP$Serotype)))
#UK_sero_freq_2 <- rep(0,length(unique(UK_PP$Serotype)))
UK_sero_freq_7 <- rep(0,length(unique(UK_PP$Serotype)))
for (i in 1:length(unique(UK_PP$Serotype))){
  UK_sero_freq_1[i] <- length(which(UK_PP[which(UK_PP$Serotype==unique(UK_PP$Serotype)[i]),]$"Time"==2006))
  #UK_sero_freq_2[i] <- length(which(UK_PP[which(UK_PP$Serotype==unique(UK_PP$Serotype)[i]),]$"Time"==2004))
  UK_sero_freq_7[i] <- length(which(UK_PP[which(UK_PP$Serotype==unique(UK_PP$Serotype)[i]),]$"Time"==2012))
}

saveRDS(UK_sero_freq_1, file = "UK_sero_freq_1.rds")
saveRDS(UK_sero_freq_7, file = "UK_sero_freq_7.rds")
```
...
UK_PPclustToIndex_dict <- 1:length(unique(UK_clusters$Cluster))
names(UK_PPclustToIndex_dict) <- (unique(UK_clusters$Cluster))

for (i in 1:nrow(UK_clusters)) {
  PPxSero_matrix[UK_PPclustToIndex_dict[as.character(UK_clusters$Cluster[i])],SerotypeToIndex_dict[UK_clusters$Serotype[i]]] <- 1
}
# matrix shows information whether PP and serotype co-occur in the dataset (1=TRUE)
# this can be used as the migration matrix:
#PPxSero_matrix_prob <- PPxSero_matrix / sum(PPxSero_matrix)
...
for (i in which(UK_clusters$SeqYear=="1998")) {
  PPsero_startpop[UK_PPclustToIndex_dict[UK_clusters$Cluster[i]],SerotypeToIndex_dict[UK_clusters$Serotype[i]]] <- PPsero_startpop[UK_PPclustToIndex_dict[UK_clusters$Cluster[i]],SerotypeToIndex_dict[UK_clusters$Serotype[i]]] + 1
}
...
PPsero_startpop4_large <- matrix(0, nrow = length(unique(UK_clusters$Cluster)), ncol = length(unique(UK_clusters$Serotype)))
PP_expand_factor <- 15000 / sum(UK_cluster_freqs_sero[[1]])
PP_expand_factor_UK_large <- 50000 / sum(UK_cluster_freqs_sero[[1]])
...
for (i in 1:nrow(PPsero_startpop4)) {
    ...
    PPsero_startpop4_large[i,] <- (sapply((UK_cluster_freqs_sero[[1]][i,] + rexp(n = length(UK_cluster_freqs_sero[[1]][i,]), rate = exp_noise)) * PP_expand_factor_UK_large, rpois, n=1)) + ceiling(PPxSero_matrix_prob2[i,])

}
saveRDS(PPsero_startpop4, "UK_PPsero_startpop.rds")
saveRDS(PPsero_startpop4_large, "UK_PPsero_startpop_large.rds")
...
ggCPP_intermed_gene_presence_absence_consensus_mean <- data.frame(matrix(0, nrow = sum(ggC_filter)+1, ncol = length(unique(PopPUNK_clusters$Cluster))+1))
ggCPP_intermed_gene_presence_absence_consensus_mean[1,-1] <- paste("SeqCl_",unique(PopPUNK_clusters$Cluster),sep = "")

for (i in unique(PopPUNK_clusters$Cluster)) {
  ggCPP_intermed_gene_presence_absence_consensus_mean[-1,(which(unique(PopPUNK_clusters$Cluster) == i))+1] <- apply(as.matrix(ggC_intermed_gene_presence_absence[-1,c(FALSE,ggCPP_Mass_seq_clusters_dict[unlist(ggC_intermed_gene_presence_absence[1,-1])]==i)]), 1, cons_genomes_mean)
}
ggCPP_intermed_gene_presence_absence_consensus_mean[-1,1] <- ggC_intermed_gene_presence_absence[-1,1]
saveRDS(ggCPP_intermed_gene_presence_absence_consensus_mean, file = "ggCPP_intermed_gene_presence_absence_consensus_mean.rds")
```
...
```{r}
# do same per serotype
PP_mass_sero_freq_1 <- rep(0,length(unique(PopPUNK_clusters$Serotype)))
PP_mass_sero_freq_2 <- rep(0,length(unique(PopPUNK_clusters$Serotype)))
PP_mass_sero_freq_3 <- rep(0,length(unique(PopPUNK_clusters$Serotype)))
