...
for (i in 1:length(UK_metadata$id)) {
  UK_numbers_year_month[12 * (UK_years[as.character(UK_SeqYear_dict[UK_metadata$Accession[i]])]-1) + UK_metadata$Month[i] ,3] <- UK_numbers_year_month[12 * (UK_years[as.character(UK_SeqYear_dict[UK_metadata$Accession[i]])]-1) + UK_metadata$Month[i] ,3] + 1
}
...
plot(UK_numbers_year_month[,1] + 1/12 * (UK_numbers_year_month[,2]-1), UK_numbers_year_month[,3])
abline(v=unique(UK_metadata$Year)-13/24, col="blue")
...
UK_cluster_freqs_winter <- vector(mode = "list", length = length(UK_winters))

for(j in 1:length(UK_winters)){
  UK_cluster_freqs_winter[[j]] <- rep(NA, no_UK_PP)
  for (i in unique(UK_PP$Cluster)){
    UK_cluster_freqs_winter[[j]][which(unique(UK_PP$Cluster) == i)] <- length(which(UK_metadata[UK_meta_IDtoInd[UK_PP[UK_PP$Cluster == i,"Taxon"]],]$Winter==UK_winters[j]))
  }
}
