...
VT_mx[,] <- y[i,j] * vaccTypes[j]
VT_mig_mx[,] <- Pop_mig[i,j] * vaccTypes[j]
update(Pop_tot_VT[]) <- sum(VT_mx[i,]) + sum(VT_mig_mx[i,])
update(Pop_tot_NVT[]) <- Pop_tot[i] - Pop_tot_VT[i]
#  + sum()

initial(Pop[,]) <- Pop_ini[i,j] # deterministic, user-based start value
initial(Pop_tot[]) <- sum(Pop_ini[i,])
VT_ini_mx[,] <- Pop_ini[i,j] * vaccTypes[j]
initial(Pop_tot_VT[]) <- sum(VT_ini_mx[i,])
initial(Pop_tot_NVT[]) <- sum(Pop_ini[i,]) - sum(VT_ini_mx[i,])
...
dim(Pop_tot_VT) <- species_no
dim(Pop_tot_NVT) <- species_no
...
dim(VT_mx) <- c(species_no,sero_no)
dim(VT_mig_mx) <- c(species_no,sero_no)
dim(VT_ini_mx) <- c(species_no,sero_no)
