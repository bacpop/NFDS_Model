...
PPsero_startpop6 <- readRDS("PPsero_startpop6.rds") # start population for model (GPSC x serotypes)
...
freq_clust <- which(rowSums(PPsero_startpop6)>(sum(PPsero_startpop6)/100)) # keep only frequent clusters (filtering only for purpose of this example)
freq_sero <- which(colSums(PPsero_startpop6)>(sum(PPsero_startpop6)/100)) # keep only frequent serotypes (filtering only for purpose of this example)
species_no_test <- length(freq_clust) # number of GPSCs
sero_no_test <- length(freq_sero) # number of serotypes
...
Genotypes_test_matrix <- readRDS("Genotypes_test_matrix.rds") # genotype matrix (pangenome matrix of GPSCs) (rows = gene clusters, columns = GPSCs)
...
migMatr_test_mtx <- readRDS("migMatr_test_mtx.rds") # immigration matrix (rows are GPSCs, columns are serotypes)
...
vaccTypes_test_mtx1 <- readRDS(file = "SeroVT.rds") # information on which types are affected by vaccine (1 = serotype affected by vaccine, 0 = serotypes not affected)
vaccTypes_test_mtx <- vaccTypes_test_mtx1[freq_sero] # filter for frequent serotypes only 
